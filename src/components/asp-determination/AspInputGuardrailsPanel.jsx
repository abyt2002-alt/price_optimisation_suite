...
const AspInputGuardrailsPanel = ({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <div className="mt-4 grid grid-cols-1 gap-3 rounded-xl border border-slate-200 bg-white p-3 xl:grid-cols-2">
        <div>
        ...
        <div>
          <label className="text-xs font-semibold uppercase tracking-wide text-slate-600">Max number of price changes</label>
          <input
            type="number"
            min={0}
            step={1}
            value={controls.maxPriceChanges}
            onChange={(event) => {
              const raw = event.target.value
              if (raw === '') {
                onControlsChange({ maxPriceChanges: '' })
                return
              }
              const parsed = Number(raw)
              onControlsChange({ maxPriceChanges: Number.isFinite(parsed) ? Math.max(0, Math.round(parsed)) : '' })
            }}
            placeholder="No cap"
            className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-800 focus:border-[#2563EB] focus:outline-none focus:ring-2 focus:ring-blue-200"
          />
        </div>
      </div>
